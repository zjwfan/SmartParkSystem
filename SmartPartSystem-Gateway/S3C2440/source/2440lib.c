...
void Port_Init(void)
{
    ...
    rGPFCON = 0x55aa;			//version zjw 注释掉
//	rGPFCON = 0x0000;
...
//  rGPGCON = 0xff95ffba;          //version zjw 注释掉
	rGPGCON = 0x00000000;
    ...
    //rGPHCON = 0x2afaaa;	//注释掉 2014-7-13 01:13:44
    rGPHCON = 0x2aaaaa;		
...
void Buzzer_Freq_Set( U32 freq )
{
	...
	rTCFG1 &= ~0xf;
	rTCFG1 |= 2;			//mux = 1/8
	rTCNTB0 = (PCLK>>7)/freq;//freq周期，10ms以上
	rTCMPB0 = rTCNTB0>>1;	// 50%占空比
	rTCON &= ~0x1f;
...
void uart_putchar(unsigned char k)
{
	if(whichUart==0)//串口0
    {
 		while(!(rUTRSTAT0&0x04));   //如果UTRSTAT 2位是0，则一直循环，直到为1（transmit buffer is empty，can transmit data）
 		rUTXH0 = k;
 	}
 	else if(whichUart==1)//串口0
    {
 		while(!(rUTRSTAT1&0x04));   //如果UTRSTAT 2位是0，则一直循环，直到为1（transmit buffer is empty，can transmit data）
 		rUTXH1 = k;
 	}
}
////*****************************////
...
{
 unsigned char c;
 while(!(rUTRSTAT0&1<<0)); // 如果UTRSTAT 0位是0（表示没有有效数据），则一直循环，     
  c = rURXH0;
 return c;
}*/
...
void uart_sendbyte(char ch)
{
    if(whichUart==0)//串口0
    {
        if(ch=='\n')//这里要发送两个字符，回车+换行
        {
            while(!(rUTRSTAT0 & 0x04));//TX 为空
            Delay(10);                 //because the slow response of hyper_terminal 
            rUTXH0='\r';//回车(CR)
        }
        while(!(rUTRSTAT0 & 0x04));   //Wait until THR is empty.TX 为空
        Delay(10);
        rUTXH0=ch;
    }
    else if(whichUart==1)
      {
        if(ch=='\n')//这里要发送两个字符，回车+换行
        {
            while(!(rUTRSTAT1 & 0x04));//TX 为空
            Delay(10);                 //because the slow response of hyper_terminal 
            rUTXH1='\r';//回车(CR)
        }
        while(!(rUTRSTAT1 & 0x04));   //Wait until THR is empty.TX 为空
        Delay(10);
        rUTXH1=ch;
    }
}
...
#define MOTOR_0  (rGPFDAT&=0xf0)

#define LEN		 3
#define SHI		 0				//1  系统挂起数；//47原自带延时//更改后：4700
#define FEN		 0									//
#define MIAO	 0
#define HAOMIAO	 10
extern int temp[4];
U32 stepper[8]={0x08,0x0c,0x04,0x06,0x02,0x03,0x01,0x09};
//*********************************************************
